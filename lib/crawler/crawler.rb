...
module AliexpressApp
  class Crawler
    def initialize orders, aliexpress, woocommerce
      @orders = orders
    ...
    def login aliexpress
    p "Efetuando login"
      @b.goto "https://login.aliexpress.com/"
      frame = @b.iframe(id: 'alibaba-login-box')
      frame.text_field(name: 'loginId').set aliexpress.email
      frame.text_field(name: 'password').set aliexpress.password
    ...
    #Método principal
    def run aliexpress
    @b = self.login aliexpress #Efetua Login
